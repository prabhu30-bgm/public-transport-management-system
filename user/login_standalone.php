...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    $stmt = $conn->prepare("SELECT id, email, password, name, status FROM users WHERE email = ?");
    ...
    if ($result->num_rows === 1) {
        ...
        if ($user['status'] === 'inactive') {
            ...
            // Verify password using password_verify OR plain text fallback for seeded users
            if (password_verify($password, $user['password']) || $password === $user['password']) {
                // If it was a plain text match, upgrade the password to a secure hash in the background
        ...
        <div class="login-card">
            ...
            <div class="login-body">
                <div class="text-center mb-4">
                    ...
                    <p class="text-muted">Welcome back! Log in to manage your bookings and travel plans.</p>
                </div>

                <?php if (isset($_GET['registered']) && $_GET['registered'] === 'success'): ?>
                    <div class="alert alert-success">
                        <i class="fas fa-check-circle me-2"></i> Registration successful! You can now login with your credentials.
                    </div>
                <?php endif; ?>

                <?php if (isset($_GET['error']) && $_GET['error'] === 'login_required'): ?>
                    <div class="alert alert-warning">
                        <i class="fas fa-exclamation-circle me-2"></i> Please login to access that page.
                    </div>
                <?php endif; ?>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            inputGroups.forEach(group => {
                ...
                input.addEventListener('focus', () => {
                    group.style.boxShadow = '0 5px 15px rgba(67, 97, 238, 0.15)';
                });
